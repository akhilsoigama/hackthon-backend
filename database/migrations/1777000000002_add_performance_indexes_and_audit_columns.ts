...
export default class extends BaseSchema {
  ...
  this.schema.alterTable('quizzes', (table) => {
    table.index(['institute_id'], 'quizzes_institute_id_idx')
    table.index(['faculty_id'], 'quizzes_faculty_id_idx')
    table.index(['department_id'], 'quizzes_department_id_idx')
    table.index(['created_at'], 'quizzes_created_at_idx')
    table.index(['is_active'], 'quizzes_is_active_idx')
    table.index(['deleted_at'], 'quizzes_deleted_at_idx')
  })
  ...
  this.schema.alterTable('assignment_uploads', (table) => {
    table.index(['student_id'], 'assignment_uploads_student_id_idx')
    table.index(['assignment_id'], 'assignment_uploads_assignment_id_idx')
    table.index(['created_at'], 'assignment_uploads_created_at_idx')
    table.index(['deleted_at'], 'assignment_uploads_deleted_at_idx')
  })

  this.schema.alterTable('faculty_leaves', (table) => {
    table.index(['faculty_id'], 'faculty_leaves_faculty_id_idx')
    table.index(['status'], 'faculty_leaves_status_idx')
    table.index(['created_at'], 'faculty_leaves_created_at_idx')
  })

  this.schema.alterTable('lectures', (table) => {
    table.index(['faculty_id'], 'lectures_faculty_id_idx')
    table.index(['department_id'], 'lectures_department_id_idx')
    table.index(['created_at'], 'lectures_created_at_idx')
  })

  this.schema.alterTable('faculties', (table) => {
    table.index(['institute_id'], 'faculties_institute_id_idx')
  })

  this.schema.alterTable('departments', (table) => {
    table.index(['institute_id'], 'departments_institute_id_idx')
  })
}
  async down() {
    this.schema.alterTable('quiz_attempts', (table) => {
