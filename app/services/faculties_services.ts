...
export default class FacultyController {
  ...
  async findAll({ searchFor }: { searchFor?: string | null } = {}) {
    try {
      ...
      const effectiveInstituteId =
        authInstituteId ||
        (Number.isFinite(requestInstituteId) && requestInstituteId > 0
          ? requestInstituteId
          : undefined)
      ...
      return {
        ...
        message:
          faculties.length > 0 ? messages.faculty_fetched_successfully : messages.faculty_not_found,
        data: faculties,
      }
    } catch (error) {
  ...
  async create() {
    try {
      ...
      const faculty = await Faculty.create({
        ...
        instituteId:
          this.isInstituteScopedUser(authUser) && authInstituteId
            ? authInstituteId
  ...
  async findOne() {
    try {
      ...
      return {
        status: !!faculty,
        message: faculty ? messages.faculty_fetched_successfully : messages.faculty_not_found,
        data: faculty,
      }
    } catch (error) {
  ...
  async updateOne() {
    try {
      ...
      if (
        !existingFaculty ||
        (this.isInstituteScopedUser(authUser) &&
          existingFaculty.instituteId !== effectiveInstituteId)
      ) {
        return {
          status: false,
  ...
  async deleteOne() {
    try {
      ...
      if (
        !faculty ||
        (this.isInstituteScopedUser(authUser) && faculty.instituteId !== effectiveInstituteId)
      ) {
        return {
          status: false,
          message: messages.faculty_not_found,
